...
void TIM_CCxChannelCmd(TIM_TypeDef* TIMx, uint16_t Channel, uint16_t ChannelState)
{
  /* 检查参数有效性 */
  assert_param(IS_TIM_ALL_PERIPH(TIMx));
  assert_param(IS_TIM_CHANNEL(Channel));
  assert_param(IS_TIM_CCx_STATE(ChannelState));

  if (ChannelState != TIM_CCx_Disable)
  {
    TIMx->CCER |= Channel;  // 使能通道
  }
  else
  {
    TIMx->CCER &= ~Channel; // 禁用通道
  }
}
